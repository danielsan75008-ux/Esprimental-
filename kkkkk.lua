...
-- ================================================================
--  FUNÇÃO: Busca key usando request() do executor (Delta)
-- ================================================================
local function getValidKey()
    -- request() é a função HTTP nativa do Delta/executores
    ...
    if not ok2 or not decoded then return nil end

    if decoded.record and decoded.record.key then
...
SecKey:Input({
    Title       = "Sua Key",
    Description = "Cole a key e clique em Verificar",
...
-- Tenta pegar o TextBox interno do Wind UI pra capturar sem precisar de Enter
task.defer(function()
    task.wait(1)
...
SecKey:Button({
    ...
    Callback    = function()
        ...
        -- Lê o texto do TextBox direto se encontrado, senão usa o Callback
        local typed = keyInput
        if keyTextBox and keyTextBox.Text ~= "" then
            typed = keyTextBox.Text
        end
        typed = typed:match("^%s*(.-)%s*$") -- remove espaços
